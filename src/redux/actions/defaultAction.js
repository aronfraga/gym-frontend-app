...
export const setPurchase = (data, localStorage) => {
	return async (dispatch) => {
		...
		while (index--) {
			if (keys[index].slice(0, 5) === 'item_')
				items.push(localStorage.removeItem(keys[index]));
		}
...
export const planToPay = (data) => {
	return async () => {
		const token = getToken().token;
		//const response = await axios.post('http://localhost:3001/payment', data, {
		const response = await axios.post(
			'https://appgymbackend-production.up.railway.app/subscription',
			data,
			{
				headers: {
					authorization: `Bearer ${token}`,
