...
const Facilities = () => {
  ...
  const [input,setInput] = useState({ name_1: "" });
	...
	function imagePreview() {
		if (loading === 'true')
			return (<div className={Style.wrapperpreview}>
			...
			<div className={Style.titleContainer}>
				<div className={Style.titleButtonWrapper}>
					...
					<div className={Style.buttonsContainer}>						
						{isAdminLogged && !isShown && <Button onClick={handlerbutton} className={Style.UploadButton} variant="contained"
						sx={{
							background: '#0d0d6b',
							'&:hover': {
						...
						{isShown && <Button disabled={activeUpload} onClick={handlerUpLoadbutton} className={Style.UploadButton} variant='outlined'
								sx={{
									color: '#0d0d6b',
									borderColor: '#0d0d6b',
									'&:hover': {
						...
						{isShown && <Button onClick={handlerGoBackButton} className={Style.UploadButton} variant='outlined'
								sx={{
									color: '#0d0d6b',
									borderColor: '#0d0d6b',
									'&:hover': {
					...
					facilitiesImages.map((image) => (
						<div className={Style.photoContainer} key={image.asset_id}>
							<div className={Style.imgContainer}>
								<p className={isShown ? Style.facilitiename2 : Style.facilitiename1}>{image.public_id.split('/')[1]}</p>
								<img
									className={isShown ? Style.photos2 : Style.photos1}
									src={image.url}
									alt='img'
								/>
								{isShown && <Button className={Style.deleteButton} value={image.public_id} variant='outlined' onClick={handlerDeletebutton}
										style={{
											position: 'absolute',
